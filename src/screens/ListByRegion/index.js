...
const ProfileScreen = ({ navigation, route }) => {
    ...
    useEffect(() => {
        axios.get(API_URL + '/victim/listByRegion/' + route.params.regionId)
            .then(({ data }) => {
    ...
    return (
        <View style={{ flex: 1, flexDirection: 'column', justifyContent: 'flex-end', }}>
            ...
            <View style={styles.content2}>
                <Text style={{ fontSize: 16, fontWeight: 'bold', marginBottom: 5 }}>List Of Victim at {route.params.regionId}</Text>
                {
                    noData == true ? (
